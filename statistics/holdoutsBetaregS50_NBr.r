...
vecMSE.betareg.S50.NBr <- matrix(0,0,0)
vecMAE.betareg.S50.NBr <- matrix(0,0,0)
vecRMSE.betareg.S50.NBr <- matrix(0,0,0)
meanMSE.betareg.S50.NBr <- c()
meanMAE.betareg.S50.NBr <- c()
meanRMSE.betareg.S50.NBr <- c()
...
model.betareg.S50.NBr <- betareg(leftover.actual ~ leftover.data)     
# fit initial model
...
# while there are covariates with p > 0.05
  ...
  model.betareg.S50.NBr <- betareg(leftover.actual ~ leftover.data.red)    
  model.betareg.S50.coef.p.NBr <- coeftest(model.betareg.S50.NBr)[,4]
}
...
for (k in 1:nrow(holdout.data.red)){
  fitted.value[k] <- model.betareg.S50.NBr$coefficients$mean[1]
  for (l in 1:ncol(holdout.data.red)){
    fitted.value[k] <- fitted.value[k] + model.betareg.S50.NBr$coefficients$mean[l+1]*holdout.data.red[k,l]
  }
...
vecMSE.betareg.S50.NBr[i] <- mean((holdout.actual - fitted.value)^2)
vecMAE.betareg.S50.NBr[i] <- mean(abs(holdout.actual - fitted.value))
vecRMSE.betareg.S50.NBr[i] <- sqrt(vecMSE.betareg.S50.NBr[i])
...
meanMSE.betareg.S50.NBr <- mean(vecMSE.betareg.S50.NBr)
meanMAE.betareg.S50.NBr <- mean(vecMAE.betareg.S50.NBr)
meanRMSE.betareg.S50.NBr <- mean(vecRMSE.betareg.S50.NBr)