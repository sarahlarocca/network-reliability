...
vecMSE.betareg.S50.NBi <- matrix(0,0,0)
vecMAE.betareg.S50.NBi <- matrix(0,0,0)
vecRMSE.betareg.S50.NBi <- matrix(0,0,0)
meanMSE.betareg.S50.NBi <- c()
meanMAE.betareg.S50.NBi <- c()
meanRMSE.betareg.S50.NBi <- c()
...
model.betareg.S50.NBi <- betareg(leftover.actual ~ leftover.data)     
# fit initial model
...
# while there are covariates with p > 0.05
  ...
  model.betareg.S50.NBi <- betareg(leftover.actual ~ leftover.data.red)    
  model.betareg.S50.coef.p.NBi <- coeftest(model.betareg.S50.NBi)[,4]
}
...
for (k in 1:nrow(holdout.data.red)){
  fitted.value[k] <- model.betareg.S50.NBi$coefficients$mean[1]
  for (l in 1:ncol(holdout.data.red)){
    fitted.value[k] <- fitted.value[k] + model.betareg.S50.NBi$coefficients$mean[l+1]*holdout.data.red[k,l]
  }
...
vecMSE.betareg.S50.NBi[i] <- mean((holdout.actual - fitted.value)^2)
vecMAE.betareg.S50.NBi[i] <- mean(abs(holdout.actual - fitted.value))
vecRMSE.betareg.S50.NBi[i] <- sqrt(vecMSE.betareg.S50.NBi[i])
...
meanMSE.betareg.S50.NBi <- mean(vecMSE.betareg.S50.NBi)
meanMAE.betareg.S50.NBi <- mean(vecMAE.betareg.S50.NBi)
meanRMSE.betareg.S50.NBi <- mean(vecRMSE.betareg.S50.NBi)