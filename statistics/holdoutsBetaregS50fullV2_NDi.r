...
for (l in 1:length(Eco.red)){
  fitted.value <- fitted.value + model.betareg.S50.NDi$coefficients$mean[l+1]*Eco.red[l]
}
fitted.value <- exp(fitted.value)/(1 + exp(fitted.value))

#Calculate mean error for this holdout sample
MSE.model.betareg.S50.Eco.NDi = mean((S50.rel.Eco - fitted.value)^2)
MAE.model.betareg.S50.Eco.NDi = mean(abs(S50.rel.Eco - fitted.value))

prediction.S50.Eco.NDi <- fitted.value
...
for (l in 1:length(Terr.red)){
  fitted.value <- fitted.value + model.betareg.S50.NDi$coefficients$mean[l+1]*Terr.red[l]
}
fitted.value <- exp(fitted.value)/(1 + exp(fitted.value))

#Calculate mean error for this holdout sample
MSE.model.betareg.S50.Terr.NDi = mean((S50.rel.Terr - fitted.value)^2)
MAE.model.betareg.S50.Terr.NDi = mean(abs(S50.rel.Terr - fitted.value))

prediction.S50.Terr.NDi <- fitted.value

################################################################################

x.red.S50.NDi <- covariates.red
remove.ind.S50.NDi <- remove.ind
IEEE118.red.S50.NDi <- IEEE118.red
IEEE300.red.S50.NDi <- IEEE300.red
Ythan.red.S50.NDi <- Ythan.red
Eco.red.S50.NDi <- Eco.red
Terr.red.S50.NDi <- Terr.red