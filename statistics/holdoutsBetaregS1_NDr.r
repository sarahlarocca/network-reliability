...
vecMSE.betareg.S1.NDr <- matrix(0,0,0)
vecMAE.betareg.S1.NDr <- matrix(0,0,0)
vecRMSE.betareg.S1.NDr <- matrix(0,0,0)
meanMSE.betareg.S1.NDr <- c()
meanMAE.betareg.S1.NDr <- c()
meanRMSE.betareg.S1.NDr <- c()
...
model.betareg.S1.NDr <- betareg(leftover.actual ~ leftover.data)     
# fit initial model
...
# while there are covariates with p > 0.05
  ...
  model.betareg.S1.NDr <- betareg(leftover.actual ~ leftover.data.red)    
  model.betareg.S1.coef.p.NDr <- coeftest(model.betareg.S1.NDr)[,4]
}
...
for (k in 1:nrow(holdout.data.red)){
  fitted.value[k] <- model.betareg.S1.NDr$coefficients$mean[1]
  for (l in 1:ncol(holdout.data.red)){
    fitted.value[k] <- fitted.value[k] + model.betareg.S1.NDr$coefficients$mean[l+1]*holdout.data.red[k,l]
  }
...
vecMSE.betareg.S1.NDr[i] <- mean((holdout.actual - fitted.value)^2)
vecMAE.betareg.S1.NDr[i] <- mean(abs(holdout.actual - fitted.value))
vecRMSE.betareg.S1.NDr[i] <- sqrt(vecMSE.betareg.S1.NDr[i])
...
meanMSE.betareg.S1.NDr <- mean(vecMSE.betareg.S1.NDr)
meanMAE.betareg.S1.NDr <- mean(vecMAE.betareg.S1.NDr)
meanRMSE.betareg.S1.NDr <- mean(vecRMSE.betareg.S1.NDr)