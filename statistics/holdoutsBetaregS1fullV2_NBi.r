...
for (l in 1:length(Eco.red)){
  fitted.value <- fitted.value + model.betareg.S1.NBi$coefficients$mean[l+1]*Eco.red[l]
}
fitted.value <- exp(fitted.value)/(1 + exp(fitted.value))

#Calculate mean error for this holdout sample
MSE.model.betareg.S1.Eco.NBi = mean((S1.rel.Eco - fitted.value)^2)
MAE.model.betareg.S1.Eco.NBi = mean(abs(S1.rel.Eco - fitted.value))

prediction.S1.Eco.NBi <- fitted.value
...
for (l in 1:length(Terr.red)){
  fitted.value <- fitted.value + model.betareg.S1.NBi$coefficients$mean[l+1]*Terr.red[l]
}
fitted.value <- exp(fitted.value)/(1 + exp(fitted.value))

#Calculate mean error for this holdout sample
MSE.model.betareg.S1.Terr.NBi = mean((S1.rel.Terr - fitted.value)^2)
MAE.model.betareg.S1.Terr.NBi = mean(abs(S1.rel.Terr - fitted.value))

prediction.S1.Terr.NBi <- fitted.value

################################################################################

x.red.S1.NBi <- covariates.red
remove.ind.S1.NBi <- remove.ind
IEEE118.red.S1.NBi <- IEEE118.red
IEEE300.red.S1.NBi <- IEEE300.red
Ythan.red.S1.NBi <- Ythan.red
Eco.red.S1.NBi <- Eco.red
Terr.red.S1.NBi <- Terr.red