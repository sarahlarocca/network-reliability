...
for (l in 1:length(Eco.red)){
  fitted.value <- fitted.value + model.betareg.S10.NBr$coefficients$mean[l+1]*Eco.red[l]
}
fitted.value <- exp(fitted.value)/(1 + exp(fitted.value))

#Calculate mean error for this holdout sample
MSE.model.betareg.S10.Eco.NBr = mean((S10.rel.Eco - fitted.value)^2)
MAE.model.betareg.S10.Eco.NBr = mean(abs(S10.rel.Eco - fitted.value))

prediction.S10.Eco.NBr <- fitted.value
...
for (l in 1:length(Terr.red)){
  fitted.value <- fitted.value + model.betareg.S10.NBr$coefficients$mean[l+1]*Terr.red[l]
}
fitted.value <- exp(fitted.value)/(1 + exp(fitted.value))

#Calculate mean error for this holdout sample
MSE.model.betareg.S10.Terr.NBr = mean((S10.rel.Terr - fitted.value)^2)
MAE.model.betareg.S10.Terr.NBr = mean(abs(S10.rel.Terr - fitted.value))

prediction.S10.Terr.NBr <- fitted.value

################################################################################

x.red.S10.NBr <- covariates.red
remove.ind.S10.NBr <- remove.ind
IEEE118.red.S10.NBr <- IEEE118.red
IEEE300.red.S10.NBr <- IEEE300.red
Ythan.red.S10.NBr <- Ythan.red
Eco.red.S10.NBr <- Eco.red
Terr.red.S10.NBr <- Terr.red