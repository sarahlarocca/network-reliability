...
vecMSE.betareg.S75.NR <- matrix(0,0,0)
vecMAE.betareg.S75.NR <- matrix(0,0,0)
vecRMSE.betareg.S75.NR <- matrix(0,0,0)
meanMSE.betareg.S75.NR <- c()
meanMAE.betareg.S75.NR <- c()
meanRMSE.betareg.S75.NR <- c()
...
model.betareg.S75.NR <- betareg(leftover.actual ~ leftover.data)     
# fit initial model
...
# while there are covariates with p > 0.05
  ...
  model.betareg.S75.NR <- betareg(leftover.actual ~ leftover.data.red)    
  model.betareg.S75.coef.p.NR <- coeftest(model.betareg.S75.NR)[,4]
}
...
for (k in 1:nrow(holdout.data.red)){
  fitted.value[k] <- model.betareg.S75.NR$coefficients$mean[1]
  for (l in 1:ncol(holdout.data.red)){
    fitted.value[k] <- fitted.value[k] + model.betareg.S75.NR$coefficients$mean[l+1]*holdout.data.red[k,l]
  }
...
vecMSE.betareg.S75.NR[i] <- mean((holdout.actual - fitted.value)^2)
vecMAE.betareg.S75.NR[i] <- mean(abs(holdout.actual - fitted.value))
vecRMSE.betareg.S75.NR[i] <- sqrt(vecMSE.betareg.S75.NR[i])
...
meanMSE.betareg.S75.NR <- mean(vecMSE.betareg.S75.NR)
meanMAE.betareg.S75.NR <- mean(vecMAE.betareg.S75.NR)
meanRMSE.betareg.S75.NR <- mean(vecRMSE.betareg.S75.NR)