...
vecMSE.betareg.S50.NDi <- matrix(0,0,0)
vecMAE.betareg.S50.NDi <- matrix(0,0,0)
vecRMSE.betareg.S50.NDi <- matrix(0,0,0)
meanMSE.betareg.S50.NDi <- c()
meanMAE.betareg.S50.NDi <- c()
meanRMSE.betareg.S50.NDi <- c()
...
model.betareg.S50.NDi <- betareg(leftover.actual ~ leftover.data)     
# fit initial model
...
# while there are covariates with p > 0.05
  ...
  model.betareg.S50.NDi <- betareg(leftover.actual ~ leftover.data.red)    
  model.betareg.S50.coef.p.NDi <- coeftest(model.betareg.S50.NDi)[,4]
}
...
for (k in 1:nrow(holdout.data.red)){
  fitted.value[k] <- model.betareg.S50.NDi$coefficients$mean[1]
  for (l in 1:ncol(holdout.data.red)){
    fitted.value[k] <- fitted.value[k] + model.betareg.S50.NDi$coefficients$mean[l+1]*holdout.data.red[k,l]
  }
...
vecMSE.betareg.S50.NDi[i] <- mean((holdout.actual - fitted.value)^2)
vecMAE.betareg.S50.NDi[i] <- mean(abs(holdout.actual - fitted.value))
vecRMSE.betareg.S50.NDi[i] <- sqrt(vecMSE.betareg.S50.NDi[i])
...
meanMSE.betareg.S50.NDi <- mean(vecMSE.betareg.S50.NDi)
meanMAE.betareg.S50.NDi <- mean(vecMAE.betareg.S50.NDi)
meanRMSE.betareg.S50.NDi <- mean(vecRMSE.betareg.S50.NDi)