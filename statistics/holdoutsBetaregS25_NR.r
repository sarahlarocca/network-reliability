...
vecMSE.betareg.S25.NR <- matrix(0,0,0)
vecMAE.betareg.S25.NR <- matrix(0,0,0)
vecRMSE.betareg.S25.NR <- matrix(0,0,0)
meanMSE.betareg.S25.NR <- c()
meanMAE.betareg.S25.NR <- c()
meanRMSE.betareg.S25.NR <- c()
...
model.betareg.S25.NR <- betareg(leftover.actual ~ leftover.data)     
# fit initial model
...
# while there are covariates with p > 0.05
  ...
  model.betareg.S25.NR <- betareg(leftover.actual ~ leftover.data.red)    
  model.betareg.S25.coef.p.NR <- coeftest(model.betareg.S25.NR)[,4]
}
...
for (k in 1:nrow(holdout.data.red)){
  fitted.value[k] <- model.betareg.S25.NR$coefficients$mean[1]
  for (l in 1:ncol(holdout.data.red)){
    fitted.value[k] <- fitted.value[k] + model.betareg.S25.NR$coefficients$mean[l+1]*holdout.data.red[k,l]
  }
...
vecMSE.betareg.S25.NR[i] <- mean((holdout.actual - fitted.value)^2)
vecMAE.betareg.S25.NR[i] <- mean(abs(holdout.actual - fitted.value))
vecRMSE.betareg.S25.NR[i] <- sqrt(vecMSE.betareg.S25.NR[i])
...
meanMSE.betareg.S25.NR <- mean(vecMSE.betareg.S25.NR)
meanMAE.betareg.S25.NR <- mean(vecMAE.betareg.S25.NR)
meanRMSE.betareg.S25.NR <- mean(vecRMSE.betareg.S25.NR)