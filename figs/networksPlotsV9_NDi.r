...
betareg.NDi.plot.data <- betareg.NDi[(betareg.NDi$var != 'Intercept') & (betareg.NDi$var != 'Phi') & (betareg.NDi$num != '50%') & (betareg.NDi$num != '75%'),]
...
dev.off()
ggsave(filename = 'betareg_NDi.pdf')
...
meanS.NDi.dfr[5,2] <- meanS_NDi_num[5]


names(meanS.NDi.dfr) <- c('Sr','num')
meanS.NDi.dfr <- meanS.NDi.dfr[(meanS.NDi.dfr$num != '50%') & (meanS.NDi.dfr$num != '75%'),]
...
vecMAE.betareg.S1.NDi <- read.table('holdoutsVecMAE_S1_NDi.txt')
...
min95.S1 <- sort(as.matrix(vecMAE.betareg.S1.NDi))[3]
...
max95.S1 <- sort(as.matrix(vecMAE.betareg.S1.NDi))[98]
