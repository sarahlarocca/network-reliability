...
betareg.NBr.plot.data <- betareg.NBr[(betareg.NBr$var != 'Intercept') & (betareg.NBr$var != 'Phi') & (betareg.NBr$num != '50%') & (betareg.NBr$num != '75%'),]
...
dev.off()
ggsave(filename = 'betareg_NBr.pdf')
...
meanS.NBr.dfr[5,2] <- meanS_NBr_num[5]


names(meanS.NBr.dfr) <- c('Sr','num')
meanS.NBr.dfr <- meanS.NBr.dfr[(meanS.NBr.dfr$num != '50%') & (meanS.NBr.dfr$num != '75%'),]
...
vecMAE.betareg.S1.NBr <- read.table('holdoutsVecMAE_S1_NBr.txt')
...
min95.S1 <- sort(as.matrix(vecMAE.betareg.S1.NBr))[3]
...
max95.S1 <- sort(as.matrix(vecMAE.betareg.S1.NBr))[98]
