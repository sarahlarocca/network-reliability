function [lMean_,lMax_,lMin_,lStd_] = calcDijkstraV4(A_,n_,networkParams)
...
for i = 1:iMax;
    for j = 1:m(i)
        ...
        lMean_{i}(j) = mean(d_(d_~=0 & d~= inf));
        lMax_{i}(j) = max(d_(d_~=0 & d~= inf));
        lMin_{i}(j) = min(d_(d_~=0 & d~= inf));
        lStd_{i}(j) = std(d_(d_~=0 & d~= inf));
    end    
end